class QueryDataset < ActiveRecord::Base
   ...
   validates :input_type, presence: true, inclusion: { in: %w(raw_reads trimmed_fasta assembly) }
   ...
   private
	 ...
	 MiGA::Dataset.new(project.miga, miga_name, false, {user: user_id, type: :genome})
	 ...
	 when :raw_reads
	    ...
	    FileUtils.copy(input_file.path,
		  ...
		  project.miga.path))
	    FileUtils.copy(input_file2.path,
	       File.expand_path("data/01.raw_reads/#{miga_name}.2.fastq",
		  project.miga.path)) if input_file_2
	    f = File.open(
