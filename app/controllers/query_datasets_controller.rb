class QueryDatasetsController < ApplicationController
   ...
   def index
	 ...
	 @running_qd = @all_qd - @ready_qd
      ...
      @query_datasets = WillPaginate::Collection.create(current_page, per_page, qd.size) do |pager|
   ...
   private
	 ...
	 params.require(:query_dataset).permit(:name, :user_id, :project_id, :input_file, :input_file_2, :input_type)
      end
